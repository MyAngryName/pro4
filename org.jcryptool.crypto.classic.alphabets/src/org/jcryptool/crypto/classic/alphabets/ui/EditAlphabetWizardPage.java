...
public class EditAlphabetWizardPage extends WizardPage implements Listener {
	 ...
	 */
	public EditAlphabetWizardPage(AbstractAlphabet alphabet) {
		super("page_name", Messages.getString("EditAlphabetWizardPage.1"), null);	 //$NON-NLS-1$ //$NON-NLS-2$
		setPageComplete(false);
		setDescription(Messages.getString("EditAlphabetWizardPage.2")); //$NON-NLS-1$
	...
	public void createControl(Composite parent) {
		...
		nameText.setText(alphabet.getName());
		charsText.setText(new String(alphabet.getCharacterSet()));
		setPageComplete(false);
	}
	...
	private boolean mayFinish() {
		if ( nameText.getText().length() > 0 && charsText.getText().length() > 0 && modified) {
			return true;
		}
		return false;
	}
	...
	private void createAlphabetNameGroup(Composite parent) {
		...
		alphabetNameGroup.setText(Messages.getString("EditAlphabetWizardPage.3")); //$NON-NLS-1$
		alphabetNameGroup.setLayoutData(alphabetNameGroupGridData);
		nameLabel = new Label(alphabetNameGroup, SWT.NONE);
		nameLabel.setText(Messages.getString("EditAlphabetWizardPage.4")); //$NON-NLS-1$
			...
			public void verifyText(VerifyEvent e) {
				...
				if (e.character != SWT.BS && e.character != SWT.DEL) {
					if (nameText.getText().length() >= 75) {
						setErrorMessage(Messages.getString("EditAlphabetWizardPage.5")); //$NON-NLS-1$
						e.doit = false;						
					}
				}
			}
		...
		maxNameCharsLabel.setText(Messages.getString("EditAlphabetWizardPage.6")); //$NON-NLS-1$
	...
	private void createAlphabetCharsGroup(Composite parent) {
		...
		alphabetCharsGroup.setText(Messages.getString("EditAlphabetWizardPage.7")); //$NON-NLS-1$
		alphabetCharsGroup.setLayoutData(alphabetCharsGroupGridData);
		charsLabel = new Label(alphabetCharsGroup, SWT.NONE);
		charsLabel.setText(Messages.getString("EditAlphabetWizardPage.8")); //$NON-NLS-1$
			...
			public void verifyText(VerifyEvent e) {
				...
				if (e.character != SWT.BS && e.character != SWT.DEL) {
					...
					if (charsText.getText().indexOf(val) != -1) {
						setErrorMessage(Messages.getString("EditAlphabetWizardPage.9")); //$NON-NLS-1$
						e.doit = false;						
					}
				}
			}
		...
		filterDescriptionLabel.setText(Messages.getString("EditAlphabetWizardPage.10")); //$NON-NLS-1$
